...
static FLOAT_POW5_INV_SPLIT: [u64; 32] = [576460752303423489,
                                          461168601842738791,
                                          368934881474191033,
                                          295147905179352826,
                                          472236648286964522,
...
static FLOAT_POW5_SPLIT: [u64; 47] = [1152921504606846976,
                                      1441151880758558720,
                                      1801439850948198400,
                                      2251799813685248000,
                                      1407374883553280000,
...
pub fn f2d(ieee_mantissa: u32, ieee_exponent: u32) -> FloatingDecimal32 {
    ...
    if e2 >= 0 {
        ...
        if q != 0 && (vp - 1) / 10 <= vm / 10 {
            ...
            last_removed_digit = (mul_pow5_inv_div_pow2(mv, q - 1, -e2 + q as i32 - 1 + l) %
                                  10) as u8;
        }
        if q <= 9 {
...
unsafe fn to_chars(v: FloatingDecimal32, sign: bool, result: *mut u8) -> usize {
    ...
    while output >= 10000 {
        ...
        let c0 = (c % 100) << 1;
        let c1 = (c / 100) << 1;
        ptr::copy_nonoverlapping(DIGIT_TABLE.get_unchecked(c0 as usize),
                                 result.offset(index + olength as isize - i - 1),
                                 2);
        ptr::copy_nonoverlapping(DIGIT_TABLE.get_unchecked(c1 as usize),
    ...
    if output >= 100 {
        let c = (output % 100) << 1;
        output /= 100;
        ptr::copy_nonoverlapping(DIGIT_TABLE.get_unchecked(c as usize),
                                 result.offset(index + olength as isize - i - 1),
                                 2);
        i += 2;
    }
    ...
    *result.offset(index) = b'E';
    ...
    if exp >= 10 {
        ptr::copy_nonoverlapping(DIGIT_TABLE.get_unchecked((2 * exp) as usize),
                                 result.offset(index),
                                 2);
        index += 2;
    } else {
...
///     let n = ryu_ecmascript::raw::f2s_buffered_n(f, &mut buffer[0]);
