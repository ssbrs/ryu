...
pub static DOUBLE_POW5_TABLE: [u64; 26] = [1,
                                           5,
                                           25,
                                           125,
                                           625,
...
pub static DOUBLE_POW5_SPLIT2: [(u64, u64); 13] = [(0, 72057594037927936),
...
pub static POW5_OFFSETS: [u32; 13] = [0x00000000, 0x00000000, 0x00000000, 0x033c55be, 0x03db77d8,
                                      0x0265ffb2, 0x00000800, 0x01a8ff56, 0x00000000, 0x0037a200,
                                      0x00004000, 0x03fffffc, 0x00003ffe];

pub static DOUBLE_POW5_INV_SPLIT2: [(u64, u64); 13] = [(1, 288230376151711744),
...
pub static POW5_INV_OFFSETS: [u32; 20] =
    [0x51505404, 0x55054514, 0x45555545, 0x05511411, 0x00505010, 0x00000004, 0x00000000,
     0x00000000, 0x55555040, 0x00505051, 0x00050040, 0x55554000, 0x51659559, 0x00001000,
     0x15000010, 0x55455555, 0x41404051, 0x00001010, 0x00000014, 0x00000000];
...
pub unsafe fn compute_inv_pow5(i: u32) -> (u64, u64) {
    ...
    let shifted_sum = ((b0 >> delta) + (b2 << (64 - delta))) + 1 +
                      ((*POW5_INV_OFFSETS.get_unchecked((i / 16) as usize) >>
                        ((i % 16) << 1)) & 3) as u128;
    (shifted_sum as u64, (shifted_sum >> 64) as u64)
}
...
pub unsafe fn compute_inv_pow5(i: u32) -> (u64, u64) {
    ...
    let delta = pow5bits(base2 as i32) - pow5bits(i as i32);
    debug_assert!(base < POW5_INV_OFFSETS.len() as u32);
    (shiftright128(low0, sum, delta) + 1 +
